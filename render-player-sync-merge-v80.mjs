...
function removeOldMergeHelpers(input) {
  const start = input.indexOf("function asObjectMap(value)");
  const readMarker = "async function readRequestBody";
  const end = input.indexOf(readMarker, start);
  if (start >= 0 && end > start && input.slice(start, end).includes("function mergePlayerSafeData")) {
...
function patchServer(server) {
  let next = removeOldMergeHelpers(server);
  ...
  return codesKeepSameCatalog(previousData.codes, nextData.codes) && Array.isArray(nextData.teams);
}`);

  next = insertAfterFunction(next, "function isPlayerSafeUpdate", `function syncMergeObjectMap(value) {
...
function syncMergePlayerSafeData(previousData, nextData) {
  ...
  return merged;
}`, "function syncMergePlayerSafeData(");
...
function replaceOnce(input, search, replacement, label) {
  if (!input.includes(search)) throw new Error(`Patch v${VERSION} introuvable: ${label}`);
...
function patchApp(app) {
  ...
  if (!next.includes('updatedAt: Date.now(),\n      answers: {},')) {
    next = replaceOnce(next, 'status: "briefing",\n      answers: {},', 'status: "briefing",\n      updatedAt: Date.now(),\n      answers: {},', 'creation equipe updatedAt');
  }
  next = next.replace('team.name = name;\n  saveData();', 'team.name = name;\n  touchTeam(team);\n  saveData();');
  next = next.replace('team.answers[puzzle.id] = input.value.trim();\n    unlockNextPuzzle(team, route, puzzle.id);\n    saveData();', 'team.answers[puzzle.id] = input.value.trim();\n    unlockNextPuzzle(team, route, puzzle.id);\n    touchTeam(team);\n    saveData();');
  next = next.replace('team.attempts[puzzle.id] = (team.attempts[puzzle.id] || 0) + 1;\n\n  if (proposed === expected) {', 'team.attempts[puzzle.id] = (team.attempts[puzzle.id] || 0) + 1;\n  touchTeam(team);\n\n  if (proposed === expected) {');
  next = next.replace('team.answers[puzzle.id] = "Photo envoyée";\n  unlockNextPuzzle(team, route, puzzle.id);\n  saveData();', 'team.answers[puzzle.id] = "Photo envoyée";\n  unlockNextPuzzle(team, route, puzzle.id);\n  touchTeam(team);\n  saveData();');
  next = next.replace('team.hints[puzzle.id] = shownCount + 1;\n  saveData();', 'team.hints[puzzle.id] = shownCount + 1;\n  touchTeam(team);\n  saveData();');
  return next;
}
...
console.log("Player sync merge v80 applied.");
