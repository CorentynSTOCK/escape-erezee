...
const TRACKING_SECTION = String.raw`        <section class="view tracking-view" id="tracking-view" aria-labelledby="tracking-title">
...
const TRACKING_FUNCTIONS = String.raw`function loadTrackingTeamFilter() {
  try {
...
function renderLiveTeamEntriesMap(container, entries, emptyMessage) {
  ...
  if (!visibleEntries.length) {
    ...
    if (layer) {
      layer.insertAdjacentHTML("beforeend", '<span class="map-empty-note">' + escapeHtml(emptyMessage || "Aucune position joueur reçue pour le moment.") + '</span>');
    }
    return;
  }
  ...
  els.trackingTeamFilters.innerHTML = entries
    .map((entry) => {
      ...
      const positionText = entry.details
        ? (entry.details.isStale ? "Ancienne position" : "Position reçue") + " · " + entry.details.ageLabel
        : "Pas de position";
      return [
...
function renderTrackingScreen() {
  ...
  if (els.trackingSummary) {
    const teamPlural = selectedIds.size === 1 ? "" : "s";
    const positionPlural = positionedCount === 1 ? "" : "s";
    els.trackingSummary.textContent = selectedIds.size + " équipe" + teamPlural + " affichée" + teamPlural + " sur " + entries.length + ". " + positionedCount + " position" + positionPlural + " reçue" + positionPlural + ".";
  }
  ...
  renderLiveTeamEntriesMap(
    els.trackingMap,
    selectedEntries,
    selectedEntries.length ? "Aucune position reçue pour les équipes sélectionnées." : "Aucune équipe sélectionnée.",
  );
}
`;

const TRACKING_STYLES = String.raw`
...
function ensureApp(app) {
  ...
  if (!next.includes('function loadTrackingTeamFilter()')) {
    next = next.replace('function renderTeamLiveMap() {', `${TRACKING_FUNCTIONS}
