...
await patchFile(INDEX_FILE, (code) => {
  let next = code
    .replace(/styles\.css\?v=\d+/g, "styles.css?v=31")
    .replace(/app\.js\?v=\d+/g, "app.js?v=31")
    .replace(/assets\/logo-escape\.svg(?:\?v=\d+)?/g, "assets/logo-escape.svg?v=31");
...
await patchFile(SERVICE_WORKER_FILE, (code) => {
  let next = code.replace(/escape-erezee-v\d+/, "escape-erezee-v31");
  next = next.replace(/\.\/assets\/logo-escape\.svg(?:\?v=\d+)?/g, "./assets/logo-escape.svg?v=31");

  if (!next.includes("./assets/logo-escape.svg?v=31")) {
