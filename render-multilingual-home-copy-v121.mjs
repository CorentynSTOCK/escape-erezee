...
/* multilingual-v121 */
(function installHomeCopyTranslationsV121() {
  if (window.__escapeI18nHomeCopyV121) return;
  window.__escapeI18nHomeCopyV121 = true;
  ...
  const introPattern = /Explorez Erezée en équipe[\s\S]*?fin du chrono\./;
  const conceptPattern = /Chaque parcours vous emmène[\s\S]*?bon moment\./;

  function patchHomeLongCopyV121() {
    ...
    nodes.forEach((node) => {
      ...
      next = next.replace(introPattern, copy.intro).replace(conceptPattern, copy.concept);
      if (next !== node.nodeValue) node.nodeValue = next;
    });
  }

  const previousApplyDomV121 = typeof escapeI18nApplyDom === 'function' ? escapeI18nApplyDom : null;
  if (previousApplyDomV121) {
    escapeI18nApplyDom = function escapeI18nApplyDomV121() {
      const result = previousApplyDomV121.apply(this, arguments);
      patchHomeLongCopyV121();
      return result;
    };
  }

  window.addEventListener('hashchange', () => window.setTimeout(patchHomeLongCopyV121, 180));
  window.setTimeout(patchHomeLongCopyV121, 0);
  window.setTimeout(patchHomeLongCopyV121, 1200);
})();
`;

function patchApp(app) {
  if (app.includes('multilingual-v121')) return bumpAssetVersions(app);
